...
function movieJsonLd(data, url) {
  const payload = {
    ...
    '@type': 'Movie',
    name: data.title,
    description: `${data.title} (${(data.release_date || '').slice(0,4)}) Ganzer Film Deutsch Stream Online anschauen. Sehen Sie Trailer, Handlung, Besetzung, Bewertungen und aktuelle Informationen auf CineBox Deutschland.`,
    url: url,
...
function tvJsonLd(data, url) {
  const payload = {
    ...
    '@type': 'TVSeries',
    name: data.name,
    description:`${data.name} (${(data.first_air_date || '').slice(0,4)}) Ganzer Film Deutsch Stream Online anschauen. Sehen Sie Trailer, Handlung, Besetzung, Bewertungen und aktuelle Informationen auf CineBox Deutschland.`,
  };
  if (data.vote_average && data.vote_count) {
    payload.aggregateRating = {
      '@type': 'AggregateRating',
...
function personJsonLd(person, url) {
  const payload = {
    ...
    '@type': 'Person',
    name: person.name,
    description: person.biography ? person.biography.slice(0, 160) : `${person.name} Biografie, Filme und Serien auf CineBox.`,
...
<style>
  ...
  .person-poster { width: 220px; flex-shrink: 0; border-radius: 12px; overflow: hidden; }
  .person-poster img { width: 100%; display: block; }
  .person-info { flex-grow: 1; }
  .person-name { font-size: 2.2rem; font-weight: 900; margin-bottom: 10px; color: #fff; }
  .person-meta { display: flex; gap: 15px; font-size: 0.9rem; color: #a1a1aa; margin-bottom: 15px; }
...
module.exports = { head, layout, posterCard, genreRow, trailerBlock, castGrid, mockPlayerBlock, escapeHtml, movieJsonLd, tvJsonLd, personJsonLd, sideBannerAd, nativeBannerAd, DEFAULT_TITLE, DEFAULT_DESC, SITE_NAME };
