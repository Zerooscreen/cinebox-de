...
function movieJsonLd(data, url) {
  const payload = {
    ...
    '@type': 'Movie',
    name: data.title,
    description: description:`${data.title} (${(data.release_date || '').slice(0,4)}) Ganzer Film Deutsch Stream Online anschauen. Sehen Sie Trailer, Handlung, Besetzung, Bewertungen und aktuelle Informationen auf CineBox Deutschland.`,
  };
...
function tvJsonLd(data, url) {
  const payload = {
    ...
    '@type': 'TVSeries',
    name: data.name,
    description: description:`${data.name} (${(data.first_air_date || '').slice(0,4)}) Ganzer Film Deutsch Stream Online anschauen. Sehen Sie Trailer, Handlung, Besetzung, Bewertungen und aktuelle Informationen auf CineBox Deutschland.`,
  };
  if (data.vote_average && data.vote_count) {
    payload.aggregateRating = {
      '@type': 'AggregateRating',
